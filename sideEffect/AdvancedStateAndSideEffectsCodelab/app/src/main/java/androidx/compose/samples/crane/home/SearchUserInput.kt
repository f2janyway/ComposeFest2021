...
fun ToDestinationUserInput(onToDestinationChanged: (String) -> Unit) {
    ...
    val currentOnDestChanged by rememberUpdatedState(newValue = onToDestinationChanged)
    LaunchedEffect(key1 = currentOnDestChanged, block = {
        snapshotFlow { editableUserInputState.text }
            .filter { !editableUserInputState.isHint }
            .collect {
                currentOnDestChanged(editableUserInputState.text)
            }
    })
}
